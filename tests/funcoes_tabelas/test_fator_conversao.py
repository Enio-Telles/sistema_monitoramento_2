...
def test_ler_fatores_manuais_missing_column(tmp_path, monkeypatch):
    ...
    # Create invalid polars dataframe (missing "unid")
...
def test_pipeline_manual_factor_override(tmp_path, monkeypatch):
    ...
    df_final_auto = pl.DataFrame({
        ...
        "v_entr_total": [0.0, 0.0, 0.0],
        "preco_medio_entrada": [0.0, 0.0, 0.0],
        "v_saida_total": [0.0, 0.0, 0.0],
        "preco_medio_saida": [0.0, 0.0, 0.0],
        "fator_de_conversao": [1.0, 2.5, 3.0] # Auto calculated factors
    })
    ...
    df_final = df_final_auto.join(df_manual, on=["chave_produto", "unidade"], how="left")
    ...
    assert list(df_final["fator_de_conversao"]) == [1.2, 5.0, 3.0]
